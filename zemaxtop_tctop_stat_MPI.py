...
#------------------------
out_path='./results_ocn_lnd_zemaxtop_cbaseNoPrec/'
  ...
  for iday_0 in range(ndays):
    ...
    for ig in grans:
      ...
      # from ECMWF-AUX
        ...
        # f. cbase_precp_flag
        #cbase_precp_flag = []
        #for ism in range(0,num_pix):
        #   if cld_lay_base[ism,0] > 0.:
        #      cbase = list(height[ism,:] > cld_lay_base[ism,0]).count(True)
        #      cbase_precp_flag.append(ze[ism,cbase] < -15.)
        #   else:
        #      cbase_precp_flag.append(False)
        #cbase_precp_flag = np.array(cbase_precp_flag)
        ...
        for ism in range(0,n_sampl):
           ...
           cbase = list(height_samp[ism,:] > cld_base_samp[ism]).count(True)
           if ze_samp[ism,cbase] >-15.: #non-precip at cloud base
              continue

           nclev = np.count_nonzero(clevs)
           if nclev > 1:
              ze_max = ze_samp[ism,clevs][0:2].max()
           else:
              ze_max = ze_samp[ism,clevs][0]
           t_ctop = tair_samp[ism,clevs][0]
           
           # locate in the t_ctop-ze_max table
