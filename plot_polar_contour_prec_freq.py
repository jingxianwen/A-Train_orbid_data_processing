...
stats_rn=get_area_mean_min_max(freq_rn[imon,latbound1:latbound2,:],lat[latbound1:latbound2])
stats_sn=get_area_mean_min_max(freq_sn[imon,latbound1:latbound2,:],lat[latbound1:latbound2])

# add cyclic

freq_rn_now=add_cyclic_point(freq_rn[imon,:,:])
freq_sn_now=add_cyclic_point(freq_sn[imon,:,:])
...
labels=["rain frequency","snow frequency"] 
units="%"
for i in range(0,2):
   #1. first plot
    levels = None
    norm = None
    cnlevels=np.linspace(10,90,9)
    ...
    if i == 0:
        dtplot=freq_rn_now[:,:]
        cmap= "twilight" #parameters["colormap"]
        stats_now=stats_rn
    elif i == 1:
        dtplot=freq_sn_now[:,:]
        cmap="twilight" #parameters["colormap"]
