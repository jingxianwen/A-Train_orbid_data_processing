...
pre_rn_tot = pre_rn_cert + pre_rn_poss
pre_sn_tot = pre_sn_cert + pre_sn_poss

freq_rn = np.divide(cnt_rn_tot,cnt_tot,where= cnt_tot!=0)*100.
freq_sn = np.divide(cnt_sn_tot,cnt_tot,where= cnt_tot!=0)*100.
rate_rn_inprec = np.divide(pre_rn_tot,cnt_rn_tot,where= cnt_rn_tot!=0)*24.
rate_sn_inprec = np.divide(pre_sn_tot,cnt_sn_tot,where= cnt_sn_tot!=0)*24.
rate_rn = rate_rn_inprec * freq_rn/100.  #np.divide(rate_rn_inprec,cnt_tot,where= cnt_tot!=0)
rate_sn = rate_sn_inprec * freq_sn/100.  #np.divide(rate_sn_inprec,cnt_tot,where= cnt_tot!=0)
...
if pole is 'N':
   var_long_name="Arctic Prec Rate ("+seasons[imon]+")"
   figure_name="Arctic_Prec_Rate_contour_"+seasons[imon]
elif pole is 'S':
   var_long_name="Antarctic Prec Rate ("+seasons[imon]+")"
   figure_name="Antarctic_Prec_Rate_contour_"+seasons[imon]
units=" " #"Fraction"
#units=r"W/m$^2$"
...
stats_rn=get_area_mean_min_max(rate_rn[imon,latbound1:latbound2,:],lat[latbound1:latbound2])
stats_sn=get_area_mean_min_max(rate_sn[imon,latbound1:latbound2,:],lat[latbound1:latbound2])

# add cyclic

rate_rn_now=add_cyclic_point(rate_rn[imon,:,:])
rate_sn_now=add_cyclic_point(rate_sn[imon,:,:])
...
labels=["rain rate","snow rate"] 
units="mm/day"
for i in range(0,2):
   #1. first plot
    levels = None
    norm = None
    cnlevels= [0.1,0.3,0.5,0.7,0.9,5.,10.,15.,20.,25.,30] #np.linspace(1,10,10)
    ...
    if i == 0:
        dtplot=rate_rn_now[:,:]
        cmap= "nipy_spectral_r" #parameters["colormap"]
        stats_now=stats_rn
    elif i == 1:
        dtplot=rate_sn_now[:,:]
        cmap="nipy_spectral_r" #parameters["colormap"]
